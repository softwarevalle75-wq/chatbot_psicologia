...
/** Resetea el estado de un cuestionario para empezarlo de nuevo */
export const resetearEstadoPrueba = async (telefono: string, tipoTest: string) => {
  const patient = await ensurePatientByPhone(telefono);
  const initial = createInitialResponses(tipoTest);
  await prisma.psychEvaluation.upsert({
    where:  { patientId_type: { patientId: patient.id, type: tipoTest } },
    update: { totalScore: 0, currentQuestion: 0, responses: initial, completed: false },
    create: { patientId: patient.id, type: tipoTest, responses: initial },
  });
};

/** Busca un practicante por número de documento */
export const buscarPracticantePorDocumento = async (documento: string) => {
  ...
  return {
    idPracticante: practitioner.id,
    nombre:        [practitioner.name, practitioner.lastName].filter(Boolean).join(" "),
    correo:        practitioner.user?.email ?? null,
    telefono:      practitioner.id, // compatibilidad
  };
};

/** Obtiene el perfil del paciente formateado para el informe PDF */
export const obtenerPerfilPacienteParaInforme = async (telefono: string) => {
  const patient = await ensurePatientByPhone(telefono);
  ...
  return {
    nombres:         full.name ?? "",
    apellidos:       full.lastName ?? "",
    documento:       full.documentNumber ?? "No disponible",
    tipoDocumento:   full.documentType  ?? "",
    telefonoPrincipal: full.whatsappNumber ?? telefono,
    semestre:        null as string | null,
