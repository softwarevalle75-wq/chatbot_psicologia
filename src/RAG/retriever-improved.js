...
const search = async (query, source, k = DEFAULT_K) => {
    ...
    const results = await client.search(collectionName, {
        ...
        // Sin score_threshold - dejar que Qdrant decida
    })
...
/**
 * Re-ranking por similitud semántica real
 * Principio: Strategy - algoritmo de re-ranking específico
 */
const rerankBySemanticSimilarity = async (query, results) => {
    if (!results || results.length === 0) return results
    
    // Generar embedding de la pregunta
    const queryVector = await generateEmbedding(query)
    
    // Calcular similitud coseno para cada resultado
    const resultsWithSimilarity = results.map(result => {
        const chunkVector = result.vector || result.payload?.vector
        
        let similarity = 0
        if (chunkVector && queryVector) {
            // Similitud coseno: (A·B) / (|A|·|B|)
            const dotProduct = queryVector.reduce((sum, a, i) => sum + a * chunkVector[i], 0)
            const magnitudeA = Math.sqrt(queryVector.reduce((sum, a) => sum + a * a, 0))
            const magnitudeB = Math.sqrt(chunkVector.reduce((sum, b) => sum + b * b, 0))
    ...
    // Ordenar por similitud semántica (no solo por score)
    return resultsWithSimilarity.sort((a, b) => b.semanticSimilarity - a.semanticSimilarity)
}
...
const smartSearch = async (query, options = {}) => {
    ...
    if (useReranking && initialResults.length > 1) {
        const rerankedResults = await rerankBySemanticSimilarity(query, initialResults)
...
export { search, smartSearch, rerankBySemanticSimilarity }
